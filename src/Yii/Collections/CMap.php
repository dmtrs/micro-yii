 ...
 * @since 1.0
 */
class CMap extends \Yii\Base\CComponent implements \IteratorAggregate,\ArrayAccess,\Countable
{
	/**
	...
	{
		return new \Yii\Collections\CMapIterator($this->_d);
	}
	...
	public function add($key,$value)
	{
		if(!$this->_r)
		{
			...
			throw new \Yii\Base\CException('The map is read only.');
	}
	...
	public function copyFrom($data)
	{
		...
		elseif($data!==null)
			throw new \Yii\Base\CException('Map data must be an array or an object implementing Traversable.');
	}
	...
	public function mergeWith($data,$recursive=true)
	{
		if(is_array($data) || $data instanceof \Traversable)
		{
			if($data instanceof \Yii\Collections\CMap)
				$data=$data->_d;
			if($recursive)
			{
