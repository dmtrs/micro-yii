...
class CMapTest extends CTestCase
{
	...
	{
		$this->map=new \Yii\Collections\CMap;
	...
	{
		$a=array(1,2,'key3'=>3);
		$map=new \Yii\Collections\CMap($a);
		$this->assertEquals(3,$map->getCount());
		$map2=new \Yii\Collections\CMap($this->map);
		$this->assertEquals(2,$map2->getCount());
	}

	public function testGetReadOnly()
	{
		$map = new \Yii\Collections\CMap(null, true);
	...
	{
		$map = new \Yii\Collections\CMap(array(), true);
	...
	{
		$map = new \Yii\Collections\CMap(array('key' => 'value'), true);
		$this->setExpectedException('\Yii\Base\CException');
		$map->remove('key');
	}
	...
	public function testMergeWith()
	{
		...
		$map=new \Yii\Collections\CMap($a);
		$map2=new \Yii\Collections\CMap($b);
		$map->mergeWith($map2);
	...
	{
		$map = new \Yii\Collections\CMap(array('key' => 'value'));
		$this->assertEquals(array('key' => 'value'), $map->toArray());
	}

	public function testIteratorCurrent()
	{
		$map = new \Yii\Collections\CMap(array('key1' => 'value1', 'key2' => 'value2'));
